...
expName = 'EEG_Protocol_Short'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Utilizador\\Desktop\\ExperimentVitor\\openBCI_aquisition\\EEG protocol\\EEG_Protocol_Short_lastrun.py',
...
Eyes_ClosedClock = core.Clock()
Close_Eyes = visual.TextStim(win=win, name='Close_Eyes',
    text='Close eyes',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
...
t = 0
Eyes_ClosedClock.reset()  # clock
frameN = -1
continueRoutine = True
routineTimer.add(60.000000)
# update component parameters for each repeat
# keep track of which components have finished
Eyes_ClosedComponents = [Close_Eyes]
for thisComponent in Eyes_ClosedComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

# -------Start Routine "Eyes_Closed"-------
while continueRoutine and routineTimer.getTime() > 0:
    # get current time
    t = Eyes_ClosedClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *Close_Eyes* updates
    if t >= 0.0 and Close_Eyes.status == NOT_STARTED:
        # keep track of start time/frame for later
        Close_Eyes.tStart = t
        Close_Eyes.frameNStart = frameN  # exact frame index
        Close_Eyes.setAutoDraw(True)
    frameRemains = 0.0 + 60- win.monitorFramePeriod * 0.75  # most of one frame period left
    if Close_Eyes.status == STARTED and t >= frameRemains:
        Close_Eyes.setAutoDraw(False)
...
run_loop = data.TrialHandler(nReps=2, method='sequential', 
...
# completed 2 repeats of 'run_loop'
